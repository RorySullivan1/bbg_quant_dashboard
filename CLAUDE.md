...
- **Platform** — full-width all-catalog performance grid (every index
  ...
  **Z-Score ranking** control row above the grid (Metric: Sharpe/Sortino/
  ...
  treemap sits the **Regime Analysis** section (v0.8.5): shared regime controls
  (a **regime-type** dropdown — Volatility / Trend / Liquidity / Rate-level —
  plus a conditional **VIX-bucket** dropdown) above two **pill sub-tabs** —
  **Risk vs Return** (a 2D per-strategy annualized vol-vs-return scatter colored
  by asset class) and **Correlation** (a per-ticker correlation heatmap with its
  own **Theme** dropdown that scopes the matrix to one theme's indices). Both
  charts condition on only the days in the selected regime bucket, computed live
  from the cache (`regime_mask` / `regime_risk_return` / `regime_correlation`),
  no BQL. **Volatility** (VIX buckets — VIX rides the single fetch via
  `REGIME_TICKERS`) is wired this cycle; **Trend / Liquidity / Rate-level** are
  inert scaffolded dropdowns that leave the charts on the unconditioned all-days
  view. Section 2 is a **treemap** nested **asset class → theme →
  ticker**, tiles **sized** and **colored** by user-selectable z-scores driven
  by **two dedicated control rows** (Tile size · Tile color), each a
  Metric (Sharpe/Sortino/Return/Vol) · Window (1W/1M/3M/6M) · Lookback
  (1Y/3Y/5Y) trio that mirrors the all-catalog grid's Z-Score ranking control
  (v0.8.x; defaults **z(6M Sharpe, 1Y)** size / **z(1W Sharpe, 1Y)** color — the
  colorbar + hover re-title to the chosen color metric/window) on a diverging
  colorscale + colorbar (v0.7.0 Workstream E). The treemap is **decoupled from
  the shared lookback toggle** (its own per-row lookback dropdowns drive it);
  the toggle still drives the factor scatter + regime charts. The factor scatter
  + regime charts re-render live on the shared lookback toggle, the treemap on
  its own six dropdowns — all no BQL.
...
| `src/layout/platform.py` | Platform-tab standalone visuals (v0.7.0, distinct from the Multi-Strategy panes). `_factor_beta_scatter`/`_update_factor_scatter` — the **3D** factor-beta scatter (`go.Scatter3d`: x = β to the equity risk premium, y = β to the term premium, z = β to the trend factor / "Trend Exposure"; one marker per strategy, one trace + legend entry per asset class colored via `_asset_class_colors` — curated `ASSET_CLASS_COLORS` then unused `LINE_PALETTE` for unmapped classes; no in-figure title, per-`scene`-axis zerolines; v0.7.1). v0.8.6 adds three translucent `go.Mesh3d` **zero-reference planes** (x=0/y=0/z=0) via `_zero_planes`/`_quad_mesh`/`_axis_bounds` (sized to the padded data bounds, `Color.CHART_AXIS` at `_ZERO_PLANE_OPACITY`, legend-less + `hoverinfo="skip"`, added before the markers in the same `batch_update`). `_treemap`/`_update_treemap` — a 3-level **asset class → theme → ticker** `go.Treemap` (sized by the clipped, floored size z-score via `_treemap_leaf_sizes` — below-average (z<0) tiles collapse to a small `_TREEMAP_SIZE_FLOOR` so area tracks strength not membership count — colored by the raw color z-score — both metric/window/lookback user-selected via the two builder control rows, v0.8.x; defaults z(6M Sharpe, 1Y) size / z(1W Sharpe, 1Y) color; `*_label` strings re-title the colorbar + the `_TREEMAP_HOVER` `.format()` template — on a token-driven diverging colorscale + colorbar; parents aggregate, `branchvalues="total"`; no in-figure title, the section header stands alone). Both compute live from the fetched cache (`equity_risk_premium`/`term_premium`/`factor_beta`, `platform_treemap_frame`), no BQL. **v0.8.5 Regime Analysis:** `_regime_scatter`/`_update_regime_scatter` — a 2D `go.Scatter` of annualized vol vs return over the regime-bucket days (per asset-class trace, via `regime_risk_return`) — and `_regime_heatmap`/`_update_regime_heatmap` — a `go.Heatmap` of the theme-scoped `regime_correlation` (token-driven diverging −1..1 scale); both take an optional indicator series + `(low, high)` bucket via the `_regime_window_mask` helper (no indicator/bucket → unconditioned all-days view), `lookback`-windowed, no BQL. The section's pill sub-tabs / shared controls / live-render closures live in `builder.py`. Imports `theme` + `..stats` + `..style`. |
...
- **One BQL call per session**. `build_app` issues a single
  ...
  tabs, the v0.7.0 Platform factor scatter/treemap, and the v0.8.5 Regime
  Analysis charts — slices from that cache. The `FACTOR_TICKERS` (v0.7.0: a
  ...
  z-axis) and the v0.8.5 `REGIME_TICKERS` (`VIX Index`, the Volatility-regime
  indicator) ride this same fetch — *no second BQL call* — and, like the
