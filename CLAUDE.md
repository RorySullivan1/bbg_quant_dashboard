...
block (always visible; Weekly Commentary above a **two-section Key
Highlights** panel — left **Market Superlatives · Past Month** board, right
**New Launches** board; v0.8.0) → **top-level pill-button tab bar** with two
tabs:
...
- **Platform** — full-width all-catalog performance grid (every index
  ...
  **Z-Score ranking** control row above the grid (Metric: Sharpe/Sortino/
  ...
  colorscale + colorbar (v0.7.0 Workstream E). Both factor sections re-render
  live on the shared lookback toggle, no BQL.
...
| `src/commentary.py`   | The two whole-catalog Key Highlights builders (v0.8.0, replacing the old mixed `build_highlights`): `build_superlatives` — one card per monthly superlative = the single most-extreme catalog index over the trailing `SUPERLATIVE_WINDOW_DAYS` (Top/Weakest performer, Strongest trend, Longest bull run, Best risk-adjusted, Steadiest, Most resilient, Strongest momentum, Best diversifier, Biggest turnaround; NaN winners skipped, ties broken by ticker) — and `build_launch_cards` — newest-first new-launch cards (`asset_class · theme · currency`, launch date, days-since-launch, simple since-launch return). Both compute from the already-fetched prices, no BQL. |
...
| `src/layout/html.py` | HTML templating: `render_template(name, /, **ctx)` (substitutes `{{key}}` in `data/templates/<name>.html`, cached read) + the `STYLE_CTX` style-token bundle; loaders/renderers `_load_disclaimer`, `_load_weekly_commentary`, `_render_weekly_commentary`, `_render_highlights` (v0.8.0: two-section — `_render_superlative_cards` + `_render_launch_cards` wrapped in `highlights_two_col`; `_superlative_value_color` remaps neutral to bright chrome text for the dark cards), `_render_error` are thin callers. Imports `theme` `_sentiment_color`. |
