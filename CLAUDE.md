...
Highlights** panel — left **Market Superlatives** board (19 superlative cards,
each with its description as a hover tooltip; the window toggle re-renders the
board live from the cache, no BQL), right **New Launches** board; the panel is
**2:1** superlatives:launches; v0.8.x) → **top-level pill-button tab bar** with
two tabs:
...
| `src/commentary.py`   | The two whole-catalog Key Highlights builders (v0.8.0, replacing the old mixed `build_highlights`): `build_superlatives(meta, prices, returns, *, window_days)` — one card per superlative = the single most-extreme catalog index over the trailing `window_days` (v0.8.x: **19 cards** — performance/trend: Top/Weakest performer, Strongest trend, Longest bull run, Biggest turnaround, Most extended; risk-adjusted: Best risk-adjusted, Best Sortino, Highest Calmar, Strongest momentum; stability: Steadiest, Most resilient, Lowest tail risk, Best diversifier; stress: Most volatile, Hardest hit, Longest losing streak, Most oversold, Most below trend; NaN winners skipped, ties broken by ticker; each card carries a plain-English `description` rendered as a hover tooltip) — and `build_launch_cards` — newest-first new-launch cards (`asset_class · theme · currency`, launch date, days-since-launch, simple since-launch return). Both compute from the already-fetched prices, no BQL; the `window_days` lets the Platform-style live toggle re-scope the board. |
