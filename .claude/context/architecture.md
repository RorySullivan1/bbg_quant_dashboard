...
| `src/layout/multi_strategy.py` | The **Multi-Strategy** tab's analysis-pane **render engine** (v0.9.12-review #156, extracted from the `build_app` monolith — mirrors the `single_strategy.py` free-function pattern). Free functions taking `(state, meta, pane, …)`: `clear_pane` (reset every figure), `render_pane` / `render_one` (render the currently-mounted view of the nine analysis types), the four benchmark-dependent helpers `_render_heatmap` / `_render_rolling_corr` / `_render_rolling_beta` / `_render_outperf` (each slices its benchmark from the cached `state.universe_prices` and memoizes on `state.memo`, no BQL), and the observer binders `bind_lazy_render` (build a view on first pick) / `bind_live_controls` (per-pane benchmark + heatmap-regime controls re-render one chart live). No widgets built here (`panes.py` owns them); `builder.py` calls these from `_recompute` + the pane wiring. |
